...
import {KubeConfig, CoreV1Api, V1Secret, V1ConfigMap} from '@kubernetes/client-node';
...
app.post('/api/apply-changes', async (req, res) => {
    try {
        const {resourceName, resourceType, namespace, direction, resourceData, originalData} = req.body;
        ...
        if (resourceType === 'Secret') {
            ...
            let payload = {
                ...
                stringData: resourceData.stringData,
            ...
            let payload = {
                binaryData: resourceData.binaryData,
                data: resourceData.data,
                metadata: {
                    name: resourceName,
                    namespace: namespace
